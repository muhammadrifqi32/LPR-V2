...
public class PurchaseRequestRepository : IPurchaseRequestRepository
{
    ...
    public async Task<PurchaseRequest?> GetPurchaseRequestWithDetailsAsync(string id, CancellationToken cancellationToken = default)
    {
        return await Query()
            .Include(x => x.Status)
    ...
    public Task<PurchaseRequest?> GetByIdWithDetailsAsync(string id, CancellationToken cancellationToken = default)
        => GetPurchaseRequestWithDetailsAsync(id, cancellationToken);

    public async Task<PurchaseRequest?> GetByIdWithProcurementRequestAsync(string id, CancellationToken cancellationToken = default)
    {
        return await Query()
            .Include(x => x.Status)
            .Include(x => x.ProcurementRequest).ThenInclude(x => x.Status)
    ...
    public Task<bool> HasDetailsAsync(string purchaseRequestId, CancellationToken cancellationToken = default)
        => _context.PurchaseRequestDetails.AnyAsync(x => x.PurchaseRequestId == purchaseRequestId, cancellationToken);

    public async Task<decimal> GetUsedQuantityByProposalDetailAsync(string proposalDetailId, string? excludedPurchaseRequestId = null, CancellationToken cancellationToken = default)
    {
        var query = _context.PurchaseRequestDetails
            .AsNoTracking()
            .Where(x => x.ProposalDetailId == proposalDetailId);

        if (excludedPurchaseRequestId.IsPresent())
            query = query.Where(x => x.PurchaseRequestId != excludedPurchaseRequestId);
