...
public class ApprovalRecordRepository : IApprovalRecordRepository
{
    ...
    public async Task<ApprovalRecord?> GetByProcurementRequestAndStageAsync(string procurementRequestId, string stageName, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<List<ApprovalRecord>> GetByProcurementRequestIdAsync(string procurementRequestId, CancellationToken cancellationToken = default)
    {
        return await Query()
            .AsNoTracking()
            .Include(x => x.ApprovalStage)
