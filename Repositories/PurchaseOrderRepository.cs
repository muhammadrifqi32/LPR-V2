...
public class PurchaseOrderRepository : IPurchaseOrderRepository
{
    ...
    public async Task<PurchaseOrder?> GetByIdWithDetailsAsync(string id, CancellationToken cancellationToken = default)
    {
        return await WithDetails()
            .FirstOrDefaultAsync(x => x.PurchaseOrderId == id, cancellationToken);
    }

    public async Task<PurchaseOrder?> GetByPurchaseRequestIdAsync(string purchaseRequestId, CancellationToken cancellationToken = default)
    {
        return await WithDetails()
            .FirstOrDefaultAsync(x => x.PurchaseRequestId == purchaseRequestId, cancellationToken);
    }

    public Task<bool> ExistsByPurchaseRequestIdAsync(string purchaseRequestId, CancellationToken cancellationToken = default)
    {
