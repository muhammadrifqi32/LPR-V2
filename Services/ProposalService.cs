...
public class ProposalService : IProposalService
{
    ...
    public async Task<ProposalResponseDto> GetByIdAsync(string id, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<ProposalResponseDto> CreateAsync(CreateProposalDto dto, CancellationToken cancellationToken = default)
    {
        ...
        var proposal = new Proposal
        {
            ...
            {
                ProposalDetailId = UlidHelper.NewUlid(),
    ...
    public async Task<ProposalResponseDto> UpdateAsync(string id, UpdateProposalDto dto, CancellationToken cancellationToken = default)
    {
        ...
        {
            ProposalDetailId = detail.ProposalDetailId.IsPresent() ? detail.ProposalDetailId! : UlidHelper.NewUlid(),
    ...
    public async Task<SubmitResponseDto> SubmitAsync(string id, CancellationToken cancellationToken = default)
    {
        var proposal = await _proposalRepository.GetProposalWithDetailsAsync(id, cancellationToken)
            ?? throw new NotFoundException("Proposal", id);
    ...
    public async Task<ProposalResponseDto> ReviewAsync(string id, ReviewProposalDto dto, CancellationToken cancellationToken = default)
    {
        var proposal = await _proposalRepository.GetProposalWithDetailsAsync(id, cancellationToken)
            ?? throw new NotFoundException("Proposal", id);
        ...
        if (dto.ReviewedBy.IsPresent())
        {
            await AddActivityLogIfPossibleAsync(dto.ReviewedBy!, DocumentType.PROPOSAL, proposal.ProposalId, "REVIEW", dto.Notes ?? $"Proposal reviewed as {finalStatus.StatusName}.", cancellationToken);
        }
    ...
    private static void ValidateCreateProposal(CreateProposalDto dto)
    {
        ...
        if (dto.RequesterId.IsMissing()) errors.Add("RequesterId is required.");
    ...
    private async Task ValidateProposalDetailsAsync(IReadOnlyList<CreateProposalDetailDto> details, CancellationToken cancellationToken)
    {
        ...
        for (var index = 0; index < details.Count; index++)
        {
            ...
            if (detail.MaterialId.IsMissing()) errors.Add($"Details[{row}].MaterialId is required.");
            if (detail.UomId.IsMissing()) errors.Add($"Details[{row}].UomId is required.");
            ValidationHelper.EnsurePositive(detail.RequestedQty, $"Details[{row}].RequestedQty", errors);

            if (detail.MaterialId.IsPresent() && !await _materialRepository.Query().AnyAsync(x => x.MaterialId == detail.MaterialId, cancellationToken))
                errors.Add($"Details[{row}].Material does not exist.");

            if (detail.UomId.IsPresent() && !await _uomRepository.QueryActive().AnyAsync(x => x.UomId == detail.UomId, cancellationToken))
    ...
    private async Task ValidateRequesterAsync(string requesterId, CancellationToken cancellationToken)
    {
        if (requesterId.IsMissing())
    ...
    private async Task EnsureStatusAllowedAsync(string currentStatusId, IEnumerable<string> allowedStatuses, string message, CancellationToken cancellationToken)
    {
        if (!await _statusLookupService.IsAnyAsync(currentStatusId, allowedStatuses, cancellationToken))
            throw new ValidationException(message);
    }

    private async Task AddActivityLogIfPossibleAsync(string accountId, DocumentType documentType, string documentId, string action, string? description, CancellationToken cancellationToken)
    {
